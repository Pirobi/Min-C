...
int loop2_fun(int j, Environment env){
  ...
  int Ti123 = 0;
  if(Ti123 <= j){
    Environment loop3_env = malloc(5 * sizeof(char*));
    if(loop3_env == NULL){
...
int loop1_fun(int i, Environment env){
  ...
  int Ti118 = 0;
  if(Ti118 <= i){
    Environment loop2_env = malloc(5 * sizeof(char*));
    if(loop2_env == NULL){
      printf("Error allocating memory for environment\n");
      exit(-1);
    }
...
int mul_fun(int l, int m, int n, double* a, double* b, double* c){
  int result;
  Environment loop1_env = malloc(5 * sizeof(char*));
  if(loop1_env == NULL){
    printf("Error allocating memory for environment\n");
    exit(-1);
  }
...
double* make_fun(int m, int n, Environment env){
  ...
  Environment init_env = malloc(2 * sizeof(char*));
  if(init_env == NULL){
    printf("Error allocating memory for environment\n");
    exit(-1);
  }
...
int main(){
  ...
  Environment make_env = malloc(1 * sizeof(char*));
  if(make_env == NULL){
    printf("Error allocating memory for environment\n");
    exit(-1);
  }
  *(make_env + 0) = (int*)dummy;
